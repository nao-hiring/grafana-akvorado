...
export const createLinter = (datasource: DataSource) => {
  ...
  return linter(async (view: EditorView) => {
    ...
    const response = await datasource.post<ApiValidationResponse>("/api/v0/console/filter/validate", JSON.stringify({ filter: code }))

    if (!response.ok) return [];

    const data: ApiValidationResponse = response.data;
    return data.errors?.map(({offset, message}) => {
